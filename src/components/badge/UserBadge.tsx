...
const UserBadge = ({nickname}:{nickname:string}) => {
  ...
  return (
    <Box sx={{width:"86%", alignItems:"center"}}>
      <Accordion square={true} sx={{ borderRadius: '2em', bgcolor:"#F6F6F6" }}>
        <AccordionSummary
          expandIcon={<ExpandMoreIcon />}
          aria-controls="panel1a-content"
          id="panel1a-header">
          <Box sx={{width:"100%", my:"0.2em"}}>
            <Box sx={{ display:"flex", mx:"0.5em", fontSize:"1.1em", fontWeight:"bold", alignItems:"center", justifyItems:"center"}}><StarsIcon sx={{color:"gold", mr:"0.3em"}}/>도전과제 전시대</Box>
            <Box sx={{display:"grid", gridTemplateColumns: "repeat(3, 1fr)", gridAutoRows: "1fr"}}>
              {bestBadge(badgeData).length>0?bestBadge(badgeData).map((badges, badgeIndex)=>(
                <Box key={badgeIndex} sx={{mx:"1em", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", textAlign:"center"}} >
                  <Tooltip placement="top" title={<div style={{fontSize:"10px"}}>{badges.description}</div>}>
                    <Box component="img" src={badges.imgUrl} alt='UserImage' 
                    sx={{ objectFit: "cover", objectPosition:"center" ,borderRadius:"50%", width:"5em", height:"5em", overflow: "hidden", m:"1em",
                    filter: "grayscale(0%)",
                    }}  />
                  </Tooltip>
                  <Box sx={{height:"10%", fontWeight:"bold", fontSize:"0.7em"}}>{badges.name}</Box>
                </Box>
        ...
        <AccordionDetails>
          {badgeData.categories.map((category, index)=>(
            <Box key={index} sx={{margin:"0.5em"}}>
              <Divider/>
              <Box sx={{mt:"0.5em",mx:"0.5em", fontSize:"1em", fontWeight:"bold"}}>{category.categoryName}</Box>
              <Box sx={{display:"grid", gridTemplateColumns: "repeat(6, 1fr)", gridAutoRows: "1fr"}}>
                {category.badges.map((badges, badgeIndex)=>(
                  <Box key={badgeIndex} sx={{mx:"1em", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", textAlign:"center"}} >
                    ...
                    <Box sx={{width:"100%",bgcolor:"lightgray", height:"1em", borderRadius:"0.2em", position:"relative" }}>
                      ...
                      <Box sx={{width:`${badges.presentValue/badges.goalValue*100>=100?100:badges.presentValue/badges.goalValue*100}%`, bgcolor:"#2E88C7", height:"1em", borderRadius:"0.2em" }}></Box>
                    </Box>
                    <Box sx={{height:"10%", fontWeight:"bold", fontSize:"0.7em"}}>{badges.name}</Box>
                  </Box>
                ))}
